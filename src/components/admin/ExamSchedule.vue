    ...
    <div class="container">
      ...
      <div class="header">
        ...
        <button class="toggle-form-btn" @click="toggleForm">
          ...
          {{ showForm ? "Close" : "Add Exam Schedule" }}
        </button>
      </div>
      ...
      <div v-if="showForm" class="form-container">
        <form @submit.prevent="submitForm" class="elegant-form">
          ...
          <div class="form-group">
            <label for="startExamTime">Start Time</label>
            <input
              type="time"
              id="startExamTime"
          ...
          <div class="form-group">
            <label for="endExamTime">End Time</label>
            <input
              type="time"
              id="endExamTime"
        ...
        <table class="table">
          ...
          <tbody>
            <tr v-for="schedule in filteredSchedules" :key="schedule.id">
              ...
              <td>{{ schedule.startExamTime }}</td>
              <td>{{ schedule.endExamTime }}</td>
            ...
            <tr v-if="filteredSchedules.length === 0">
              <td colspan="5" class="no-data">No Exam Schedules Found</td>
            </tr>
          </tbody>
        </table>
      </div>
...
export default {
  ...
  setup() {
    const baseURL = "http://localhost:8081/api/v1/exam-schedule";
    ...
    const fetchSchedules = async () => {
      try {
        const response = await axios.get(`${baseURL}/getAllExamSchedules`);
    ...
    const submitForm = async () => {
      try {
        if (isEditing.value) {
          ...
          await axios.put(
            `${baseURL}/updateExamScheduleById/${form.value.id}`,
            form.value
          );
          alert("Exam Schedule updated successfully!");
        } else {
          // Add new schedule
          await axios.post(`${baseURL}/addExamSchedule`, form.value);
    ...
    if (confirm('Are you sure you want to delete this exam schedule?')) {
        try {
            console.log(`Attempting to delete ExamSchedule with ID: ${id}`);
            const response = await axios.delete(`${baseURL}/deleteExamScheduleById/${id}`);
