    ...
    <table v-if="studentRequests.length" class="requests-table">
      <thead>
        <tr>
          ...
          <th>Attachment File</th> <!-- New column for the attachment -->
          <th>Actions</th>
        </tr>
      </thead>
      <tbody>
        <tr v-for="(request, index) in studentRequests" :key="request.id">
          ...
          <td>
            <a v-if="request.attachmentFile" :href="`http://localhost:8081/images/${request.attachmentFile}`"
              target="_blank">
              View Attachment
            </a>
...
export default {
  ...
  methods: {
    ...
    async fetchStudentRequests() {
      try {
        const response = await axios.get('http://localhost:8081/api/v1/student-requests/getAllStudentRequests');
    ...
    async updateRequestStatus(requestId, request) {
      try {
        ...
        const response = await axios.put(`http://localhost:8081/api/v1/student-requests/updateStudentRequest/${requestId}/${userId}`, request);
